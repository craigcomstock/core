...
case $DISTRO in
    Ubuntu|Debian)
        ...
        ;;
    RedHatEnterprise*|SUSE|CentOS|AmazonLinux)
...
case $DISTRO in
    Ubuntu)
        ...
        if [ "$MAJOR_RELEASE" -eq 22 ]; then
            ...
            echo "Installing the package that is most likely to work."
            # Ubuntu >20
            folder="ubuntu_20_x86_64"
            comp_tag="ubuntu20"
        else
            echo "Sorry, we do not support hub functionality on $DISTRO $MAJOR_RELEASE."
            echo "Please try a supported release."
            exit 1
        fi
        ;;
    Debian)
        ...
        if [ "$MAJOR_RELEASE" -lt 7 ]; then
            echo "Sorry, we do not support hub functionality on $DISTRO $MAJOR_RELEASE."
            echo "Please try upgrading to a newer release"
            exit 1
    ...
    AmazonLinux)
        ...
        ;;
    SUSE)
        echo "Error: SUSE is not supported as an Enterprise hub platform."
        exit 1
        ;;
    *)
...
case $DISTRO in
    Ubuntu)
        ...
        if [[ "$uname_arch" != "x86_64" ]] && [[ "$uname_arch" != "aarch64" ]]; then
            ...
            echo "Sorry $DISTRO $MAJOR_RELEASE is not supported."
        fi
        ;;
    Debian)
        MAJOR_RELEASE=$(echo $RELEASE | cut -d "." -f 1)
    ...
    RedHatEnterprise*|SUSE|CentOS|AmazonLinux)
        # Red hat
        MAJOR_RELEASE=$(echo $RELEASE | cut -d "." -f 1)
        if [[ "$DISTRO" = RedHatEnterprise* || "$DISTRO" = CentOS ]] && [ "$MAJOR_RELEASE" -ge 8 ]; then
            folder="agent_rhel8_x86_64"
            comp_tag="el8"
        elif [[ "$DISTRO" = RedHatEnterprise*  || "$DISTRO" = CentOS ]] && [ "$MAJOR_RELEASE" = 7 ]; then
            folder="agent_rhel7_x86_64"
            comp_tag="el7"
        elif [[ "$DISTRO" = "AmazonLinux"  ]] && [ "$MAJOR_RELEASE" = 2 ]; then
            folder="agent_rhel7_x86_64"
            comp_tag="el7"
        elif [[ "$DISTRO" = RedHatEnterprise*  || "$DISTRO" = CentOS ]] && [ "$MAJOR_RELEASE" = 6 ]; then
